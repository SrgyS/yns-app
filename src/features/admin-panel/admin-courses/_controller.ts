...
import { createAdminAbility } from '../admin-users/_domain/ability'
import { StaffPermissionService } from '../admin-users/_services/staff-permissions'
import {
  CourseQueryInput,
  courseQuerySchema,
  CourseUpsertInput,
...
export class AdminCoursesController extends Controller {
    ...
    return {
      ...
      dailyPlans:
        course.dailyPlans?.map((plan: any) => ({
          ...
          description: plan.description,
          warmupId: plan.warmup.slug,
          mainWorkoutId: plan.mainWorkout?.slug ?? null,
    ...
    if (input.id) {
      return dbClient.course.findUnique({
        ...
        include: {
          ...
          dailyPlans: {
            include: {
              warmup: { select: { id: true, slug: true, title: true } },
              mainWorkout: { select: { id: true, slug: true, title: true } },
  ...
  private async lookupWorkouts(input: LookupQueryInput): Promise<WorkoutLookupItem[]> {
  ...
  private async lookupRecipes(input: LookupQueryInput): Promise<RecipeLookupItem[]> {
    const search = input.search?.trim()
  ...
  private async resolveWorkoutBySlug(slug: string, tx: Prisma.TransactionClient) {
    const workout = await tx.workout.findUnique({ where: { slug } })
    if (!workout || workout.needsReview) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: `Тренировка со slug "${slug}" не готова к использованию или не найдена`,
      })
    }
    return workout
  }

  private async resolveRecipeBySlug(slug: string, tx: Prisma.TransactionClient) {
  ...
  private async upsertCourse(input: CourseUpsertInput): Promise<AdminCourseDetail> {
    this.validateProduct(input)
    const allowedWorkoutDays = this.normalizeAllowedWorkoutDays(input)

    const course = await dbClient.$transaction(async (tx: Prisma.TransactionClient) => {
      const productData =
      ...
      await tx.mealPlan.deleteMany({
        where: { courseId: upsertedCourse.id, slug: { notIn: mealPlanSlugs } },
      })

      for (const plan of input.mealPlans) {
        const breakfast = await this.resolveRecipeBySlug(plan.breakfastRecipeId, tx)
      ...
      await tx.dailyPlan.deleteMany({
        where: { courseId: upsertedCourse.id, slug: { notIn: dailyPlanSlugs } },
      })

      for (const plan of input.dailyPlans) {
        const warmup = await this.resolveWorkoutBySlug(plan.warmupId, tx)
        const mainWorkout = plan.mainWorkoutId
          ? await this.resolveWorkoutBySlug(plan.mainWorkoutId, tx)
          : null
        const mealPlan = plan.mealPlanId
          ? await this.resolveMealPlanBySlug(upsertedCourse.id, plan.mealPlanId, tx)
          : null
    ...
    if (!updatedCourse) {
      throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Course not found after upsert' })
    }
