...
import { CourseBlock, VideoBlock } from '@/kernel/domain/course-page'
...
import { EquipmentBlockComponent } from './blocks/equipment-block'
...
export function BlockRenderer({ blocks, course }: BlockRendererProps) {
  ...
  const getEmbeddedVideoBlock = (
    currentBlock: CourseBlock,
    index: number
  ...
  return (
    <div className="space-y-8">
      {visibleBlocks.map((block, index) => {
        ...
        if (block.type === 'video' && previousBlock?.type === 'hero') {
          return null
        }

        const embeddedVideo = getEmbeddedVideoBlock(block, index)
        ...
        switch (block.type) {
          ...
          case 'equipment':
            return <EquipmentBlockComponent key={block.id} {...block} />
          case 'highlights':
