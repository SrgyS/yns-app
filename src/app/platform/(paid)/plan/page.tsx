...
import type { SelectableCourse } from '@/features/course-enrollment/_types/selectable-course'

function NoAccessState() {
  return (
    <section className="mx-auto flex w-full max-w-[640px] flex-col space-y-6 px-3 py-6 sm:py-17 sm:px-4 md:px-6">
      <NoAccessCallout
...
function getSelectableCourses(
  accessibleEnrollments: Awaited<
    ReturnType<GetAccessibleEnrollmentsService['exec']>
  >
): SelectableCourse[] {
  return accessibleEnrollments
...
async function activateSingleEnrollment(
  enrollmentId: string
): Promise<boolean> {
  const activateEnrollmentService = server.get(ActivateEnrollmentService)
  try {
    await activateEnrollmentService.exec(enrollmentId)
    return true
  } catch (error) {
    logger.error({
      msg: '[PlatformPlanPage] Failed to activate single accessible enrollment',
      enrollmentId,
      error,
    })
    return false
...
export default async function PlatformPlanPage() {
  ...
  if (accessibleEnrollments.length === 1) {
    ...
    if (courseSlug) {
      ...
      const shouldRedirect = await activateSingleEnrollment(
        single.enrollment.id
      )
      if (shouldRedirect) {
        redirect(`/platform/day/${courseSlug}`)
      }
  ...
  const selectableCourses = getSelectableCourses(accessibleEnrollments)

  if (selectableCourses.length === 0) {
    return <UnavailableCoursesState />
  }

  return <CourseSelectionState courses={selectableCourses} />
}
