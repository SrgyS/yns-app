...
export class SupabaseStorage {
  private supabase = createClient(privateConfig.SUPABASE_URL, privateConfig.SUPABASE_SERVICE_KEY)
  ...
  async upload(file: File, bucket: string, tag: string, userId: string): Promise<StoredFile> {
    ...
    const { error } = await this.supabase.storage.from(bucket).upload(key, file, {
      upsert: true,
