...
// Основная функция
async function downloadAndGenerateTypes(schemaDir: string, outputDir: string): Promise<void> {
  const schemaFiles = ['manifest.schema.json', 'course.schema.json', 'lesson.schema.json']
...
if (!schemaDir || !outputDir) {
  console.error('Необходимо указать оба пути: путь к схемам и путь для выходных файлов')
  process.exit(1)
}
